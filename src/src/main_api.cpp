...
void  CLink::calculate_dynamic_VDFunction(int inner_iteration_number, bool congestion_bottleneck_sensitivity_analysis_mode, int VDF_type_no)
{
    ...
    if (VDF_type_no == 0 || VDF_type_no == 1)  // BPR, VDF
    {
        ...
        for (int tau = 0; tau < assignment.g_number_of_demand_periods; ++tau)
        {
            float volume_to_be_assigned = PCE_volume_per_period[tau] + VDF_period[tau].preload + VDF_period[tau].sa_volume;
                ...
                VDF_period[tau].volume = volume_to_be_assigned/max(0.1, number_of_lanes);
    ...
    if (pLink->link_type >= 0)  // do not need to be the cap reduced link
           {
                    ...
                    pLink->RT_speed_vector[time_slot_no] = pLink->link_distance_VDF / max(0.00001, pLink->RT_travel_time / 60.0);  // mph                }
