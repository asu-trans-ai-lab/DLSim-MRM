...
void write_default_setting_file_if_not_exist()
{
	...
	myfile << "[assignment] ,,,column_generation_iterations,column_updating_iterations,odme_iterations,sensitivity_analysis_iterations,simulation_iterations,number_of_memory_blocks,remarks" << endl;
	myfile << ",,,5,0,0,0,0,1," << endl;
	...
	//myfile << ",8,traveler,traveler,traveler,10,0,1,1,1,1,0," << endl;
	myfile << "" << endl;
	...
	myfile << "[real_time_info],info_updating_freq_in_min,visual_distance_in_cells," << endl;
	myfile << ",5,5," << endl;

	myfile << "[scenario],scenario_type,notes,from_node_id,to_node_id,time_period,capacity,demand_period,response_rate," << endl;
	myfile << ",incident,,0,0,0700_0800,0,AM7" << endl;
	myfile << ",dms,,0,0,0700_0800,0,AM7,0.3" << endl;
	...
	if (parser_settings.OpenCSVFile("settings.csv", false))
	{
		while (parser_settings.ReadRecord_Section())
		{
			if (parser_settings.SectionName.compare("assignment") >= 0);
			{
				...
				if (column_generation_iterations == 0)
				{
				...
				if (sensitivity_analysis_iterations == 0 && column_updating_iterations == 0 && ODME_iterations == 0 && simulation_iterations ==0)
					assignment_mode = 0;
