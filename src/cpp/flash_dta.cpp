...
void write_default_setting_file_if_not_exist()
{
	...
	myfile << ",1,input_matrix.csv,,matrix,AM,auto,0.2,1," << endl;
	myfile << ",2,input_matrix.csv,,matrix,AM,walk,0.2,1," << endl;
	myfile << ",3,input_matrix.csv,,matrix,AM,bike,0.2,1," << endl;
	myfile << ",4,input_matrix.csv,,matrix,AM,bus,0.2,1," << endl;
	...
	if (parser_settings.OpenCSVFile("settings.csv", false))
	{
		while (parser_settings.ReadRecord_Section())
		{
			if (parser_settings.SectionName.compare("assignment") >= 0);
			{
				...
				if (assignment_mode_str == "lue" || assignment_mode_str == "ue")
					assignment_mode = 0;
